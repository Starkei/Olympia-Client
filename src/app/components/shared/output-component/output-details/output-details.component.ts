...
export class OutputDetailsComponent implements OnInit {
        ...
        this.afs
          ...
          .subscribe(
            (d: Output): void => {
              ...
              this.collection = data.get("collection");
              this.afs.collection(this.collection).doc(this.uid).valueChanges().subscribe(doc => {
                if (doc["comments"])
                  this.messages = doc["comments"];
              });
  ...
  addComment() {
    let sub = this.afs.collection<Output>(this.collection).doc(this.uid).valueChanges().subscribe(
      data => {
        if (!data["comments"])
          data["comments"] = [];
        data["comments"].push({
        ...
        this.afs.collection<Output>(this.collection).doc(this.uid).set(data);
        sub.unsubscribe();
      }
    )
  }
