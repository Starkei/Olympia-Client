...
export class FilterGenerator<T> extends FieldGenerator implements Filter {
   ...
   */
  protected getSetOfPropertiesValues(collection: Array<T>, propertyName: string): Set<string> {
    let values: Set<string> = new Set();

    for (const element of collection) {
      if (element.hasOwnProperty(propertyName)) values.add(element[propertyName]);
    }
   ...
   * @memberof FilterGenerator
   */
  protected getSetFromArrayPropertiesValues(collection: Array<T>, propertyName: string): Set<string> {
    let values: Set<string> = new Set();
